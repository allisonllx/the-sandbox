...
2. **Startup: Triage & de-risk** — The AI PM ranks the backlog by Severity, Friction, and Sensitivity, suggests an **innovation track** (Technical or Product Feature for MVP), and applies Relaxation Controls. The CTO dashboard sidebar groups items into **In triage** (pending/reviewing/approved), **Live challenges** (published), and **Closed** (submissions archived). **Preview Changes** runs the **Challenge Factory** for non-demo technical items: a single-pass **TechnicalChallengeSpec** (archetype + interface contract + typed **examples**) drives starter generation and validation; Micro-PRD is projected from the spec via `spec_to_microprd()` (assignment-style markdown: scenario, background, constraints, examples, task, DoD). Founders see internal `brand_proxy`; students receive a **Company Tech Profile** only (blind audition). Preview builds `PublishDraft` **after** spec projection (not from legacy fallback copy). Preview returns `challenge_draft` + `challenge_package` (+ optional `challenge_spec`) — founders edit title, context, blueprint override, success criteria, and company profile before **Approve & Publish**. After publish, items leave the triage list; founders can **Close submissions** to remove the challenge from the student hub while keeping Match Radar.
...
5. **Startup: Review matches** — Track-aware assessor plugins score submissions (Technical: tests + taste; Product: structure + DESIGN.md rubric). Scorecard dimensions differ by track. After publish, sponsors open **Sponsor Match Radar** at `/startup/matches/{challengeId}` — ranked performers for **that challenge only** (live submissions or demo stubs). **Live rows are clickable** → `/startup/matches/{id}/submissions/{submissionId}` for read-only code + scorecard review. Match Radar remains available after **Close submissions**. They do not see other companies' challenges or the student global leaderboard.
...
- **Shipped (hackathon):** Founder upload UI (`/startup/upload`), `POST /triage/intake`, dynamic challenge factory (TechnicalChallengeSpec + 8 system-module archetypes at Preview), CTO sidebar triage/live/closed sections + `POST /triage/close/{id}`.
