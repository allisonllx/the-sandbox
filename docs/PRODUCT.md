...
2. **Startup: Triage & de-risk** — The AI PM ranks the backlog by Severity, Friction, and Sensitivity, suggests an **innovation track** (Technical or Product Feature for MVP), and applies Relaxation Controls. **Preview Changes** runs the **Challenge Factory** for non-demo technical items: a single-pass **TechnicalChallengeSpec** (archetype + interface contract + typed **examples**) drives starter generation and validation; Micro-PRD is projected from the spec via `spec_to_microprd()` (assignment-style markdown: scenario, background, constraints, examples, task, DoD). Founders see internal `brand_proxy`; students receive a **Company Tech Profile** only (blind audition). Preview builds `PublishDraft` **after** spec projection (not from legacy fallback copy). Preview returns `challenge_draft` + `challenge_package` (+ optional `challenge_spec`) — founders edit title, context, blueprint override, success criteria, and company profile before **Approve & Publish**.
