...
export default function ChallengeWorkspacePage() {
  ...
  async function handleSubmit(code: string) {
    ...
    setError(null);
    try {
      const res = await api.submitSolution(challengeId, code);
      setSubmitResult(res);
    } catch (e) {
    ...
    <div className="min-h-screen bg-surface flex flex-col">
      <header className="flex-shrink-0 border-b border-surface-border bg-surface-raised px-6 py-3">
        <div className="flex items-center justify-between max-w-[1600px] mx-auto">
          ...
          >
            {downloading ? "Downloading…" : "Download Dataset (.sqlite)"}
          </button>
        </div>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden max-w-[1600px] mx-auto w-full">
        {/* Left — Micro-PRD */}
        <aside className="w-[380px] flex-shrink-0 border-r border-surface-border overflow-y-auto p-6">
          <MicroPRDView prd={challenge.microprd} />
          {challenge.dataset_anomalies.length > 0 && (
